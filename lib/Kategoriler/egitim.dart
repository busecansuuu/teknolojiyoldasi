...
import 'package:bitirme_webscraping/egitim_scraping.dart';
...
class _EgitimPageState extends State<EgitimPage> {
  final EgitimWebScraping webScraping = EgitimWebScraping();
  int _currentPage = 1;
  ...
  Future<void> _fetchData(int startPage, int endPage) async {
    ...
    try {
      List<Haber> newData =
          await webScraping.extractEgitimData(startPage, endPage);
      setState(() {
    ...
    return Scaffold(
      ...
      appBar: PreferredSize(
        ...
        child: Container(
          ...
          child: AppBar(
            title: const Text('Eğitim', style: TextStyle(color: Colors.white)),
            iconTheme: const IconThemeData(color: Colors.white),
            actions: [
              IconButton(
                icon: const Icon(Icons.search, color: Colors.white),

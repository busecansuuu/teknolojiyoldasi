...
import 'package:bitirme_webscraping/yazilim_scraping.dart';
import 'package:flutter/material.dart';

class YazilimPage extends StatefulWidget {
  const YazilimPage({super.key});

  @override
  _YazilimPageState createState() => _YazilimPageState();
}

class _YazilimPageState extends State<YazilimPage> {
  final YazilimWebScraping webScraping = YazilimWebScraping();
  int _currentPage = 1;
  ...
  Future<void> _fetchData(int startPage, int endPage) async {
    ...
    try {
      List<Haber> newData =
          await webScraping.extractYazilimData(startPage, endPage);
      setState(() {
    ...
    return Scaffold(
      ...
      appBar: PreferredSize(
        ...
        child: Container(
          ...
          child: AppBar(
            title: const Text('Yazılım', style: TextStyle(color: Colors.white)),
            iconTheme: const IconThemeData(color: Colors.white),
            actions: [
              IconButton(
                icon: const Icon(Icons.search, color: Colors.white),

...
import 'package:bitirme_webscraping/teknoloji_scraping.dart';
import 'package:flutter/material.dart';

class TeknolojiPage extends StatefulWidget {
  const TeknolojiPage({super.key});

  @override
  _TeknolojiPageState createState() => _TeknolojiPageState();
}

class _TeknolojiPageState extends State<TeknolojiPage> {
  final TeknolojiWebScraping webScraping = TeknolojiWebScraping();
  int _currentPage = 1;
  ...
  Future<void> _fetchData(int startPage, int endPage) async {
    ...
    try {
      List<Haber> newData =
          await webScraping.extractTeknolojiData(startPage, endPage);
      setState(() {
    ...
    return Scaffold(
      ...
      appBar: PreferredSize(
        ...
        child: Container(
          ...
          child: AppBar(
            title:
                const Text('Teknoloji', style: TextStyle(color: Colors.white)),
            iconTheme: const IconThemeData(color: Colors.white),
            actions: [
              IconButton(
                icon: const Icon(Icons.search, color: Colors.white),
